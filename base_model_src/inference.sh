...
do
  ...
  do
    ...
    do
        python inference.py --method $method --model_name $model_name --class_type $data_set --dataset_name $dataset_name --n_shot $n_shot --ep_num $ep_num --seed $seed --cross
    done
  done
done

# Simpleshot Models
for model_name in 'Conv4' 'ResNet34' 'DenseNet121' 'WideRes'
do
  for data_set in "base" "val" "novel"
  do
